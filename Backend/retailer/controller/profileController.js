...
// Turn joined rows into one user record with a unique role list.
...
// Return the retailer profile for the authenticated user.
...
// Update the retailer profile and then return the fresh record.
const updateRetailerProfile = async (req, res) => {
  try {
    ...
    const duplicate = await query('SELECT user_id FROM users WHERE email = ? AND user_id <> ?', [
      email,
      userId,
    ]);
    ...
    await query('UPDATE users SET name = ?, email = ? WHERE user_id = ?', [name, email, userId]);
